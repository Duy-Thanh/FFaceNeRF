...
class SelfAttn(torch.nn.Module):
    def __init__(self, hidden_dim, num_heads, dropout=0.1):
        ...
        self.mha = torch.nn.MultiheadAttention(hidden_dim, num_heads=num_heads, dropout=dropout)

    def forward(self, x):
        x= x.unsqueeze(0)
        #x = x.permute(1, 0, 2)  # Change shape to (seq_len, batch, embed_dim)
        attn_output, _ = self.mha(x, x, x)
        attn_output = attn_output.squeeze()  # Change back shape to (batch, seq_len, embed_dim)
        
        return attn_output


@persistence.persistent_class
class TriPlaneGenerator_simple(torch.nn.Module):
    def __init__(self,
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        if not self.disable_alignment:
            self.decoder = DisentangledOSGDecoder(32, {'decoder_lr_mul': rendering_kwargs.get('decoder_lr_mul', 1), 'decoder_output_dim': 32, 'decoder_seg_dim': 15},network_type='simple')
        else:
...
class TriPlaneGenerator_base(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        self.superresolution = dnnlib.util.construct_class_by_name(class_name=rendering_kwargs['superresolution_module'], channels=32, img_resolution=img_resolution, sr_num_fp16_res=sr_num_fp16_res, sr_antialias=rendering_kwargs['sr_antialias'], **sr_kwargs)
        if not self.disable_alignment:
            self.decoder = DisentangledOSGDecoder(32, {'decoder_lr_mul': rendering_kwargs.get('decoder_lr_mul', 1), 'decoder_output_dim': 32, 'decoder_seg_dim': 15},network_type='base')
        else:
            self.decoder = SegmentationOSGDecoder(32, {'decoder_lr_mul': rendering_kwargs.get('decoder_lr_mul', 1), 'decoder_output_dim': 32, 'decoder_seg_dim': 15})
        self.neural_rendering_resolution = 64
        self.rendering_kwargs = rendering_kwargs
...
class TriPlaneGenerator_eyes(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        self.superresolution = dnnlib.util.construct_class_by_name(class_name=rendering_kwargs['superresolution_module'], channels=32, img_resolution=img_resolution, sr_num_fp16_res=sr_num_fp16_res, sr_antialias=rendering_kwargs['sr_antialias'], **sr_kwargs)
        if not self.disable_alignment:
            self.decoder = DisentangledOSGDecoder(32, {'decoder_lr_mul': rendering_kwargs.get('decoder_lr_mul', 1), 'decoder_output_dim': 32, 'decoder_seg_dim': 15},network_type='eyes')
        else:
            self.decoder = SegmentationOSGDecoder(32, {'decoder_lr_mul': rendering_kwargs.get('decoder_lr_mul', 1), 'decoder_output_dim': 32, 'decoder_seg_dim': 15})
        self.neural_rendering_resolution = 64
        self.rendering_kwargs = rendering_kwargs
...
class TriPlaneGenerator_nose(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        self.superresolution = dnnlib.util.construct_class_by_name(class_name=rendering_kwargs['superresolution_module'], channels=32, img_resolution=img_resolution, sr_num_fp16_res=sr_num_fp16_res, sr_antialias=rendering_kwargs['sr_antialias'], **sr_kwargs)
        if not self.disable_alignment:
            self.decoder = DisentangledOSGDecoder(32, {'decoder_lr_mul': rendering_kwargs.get('decoder_lr_mul', 1), 'decoder_output_dim': 32, 'decoder_seg_dim': 15},network_type='nose')
        else:
            self.decoder = SegmentationOSGDecoder(32, {'decoder_lr_mul': rendering_kwargs.get('decoder_lr_mul', 1), 'decoder_output_dim': 32, 'decoder_seg_dim': 15})
        self.neural_rendering_resolution = 64
        self.rendering_kwargs = rendering_kwargs
...
class TriPlaneGenerator_chin(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        self.superresolution = dnnlib.util.construct_class_by_name(class_name=rendering_kwargs['superresolution_module'], channels=32, img_resolution=img_resolution, sr_num_fp16_res=sr_num_fp16_res, sr_antialias=rendering_kwargs['sr_antialias'], **sr_kwargs)
        if not self.disable_alignment:
            self.decoder = DisentangledOSGDecoder(32, {'decoder_lr_mul': rendering_kwargs.get('decoder_lr_mul', 1), 'decoder_output_dim': 32, 'decoder_seg_dim': 15},network_type='chin')
        else:
            self.decoder = SegmentationOSGDecoder(32, {'decoder_lr_mul': rendering_kwargs.get('decoder_lr_mul', 1), 'decoder_output_dim': 32, 'decoder_seg_dim': 15})
        self.neural_rendering_resolution = 64
        self.rendering_kwargs = rendering_kwargs
...
class SegmentationOSGDecoder(torch.nn.Module):
    def __init__(self, n_features, options):
        ...
        self.seg_net_appdx = FullyConnectedLayer(1 + options['decoder_seg_dim'], 17, lr_multiplier=options['decoder_lr_mul'])
...
class DisentangledOSGDecoder(torch.nn.Module):
    def __init__(self, n_features, options,network_type='simple'):
        super().__init__()
        self.hidden_dim = 64
        ...
        if network_type=='simple':
            self.geo_net_appdx = torch.nn.Sequential(
            FullyConnectedLayer(1 + options['decoder_seg_dim'], self.hidden_dim, lr_multiplier=options['decoder_lr_mul']),
            torch.nn.Softplus(),
            FullyConnectedLayer(self.hidden_dim, 19, lr_multiplier=options['decoder_lr_mul'])
            )            
        elif network_type == 'base':
            self.geo_net_appdx = torch.nn.Sequential(
            FullyConnectedLayer(1 + options['decoder_seg_dim']+32+3, self.hidden_dim, lr_multiplier=options['decoder_lr_mul']),
            torch.nn.Softplus(),
            FullyConnectedLayer(self.hidden_dim, 17, lr_multiplier=options['decoder_lr_mul'])
            )
        elif network_type == 'nose' or network_type == 'eyes':
            self.geo_net_appdx = torch.nn.Sequential(
            FullyConnectedLayer(1 + options['decoder_seg_dim']+32+3, self.hidden_dim, lr_multiplier=options['decoder_lr_mul']),
            torch.nn.Softplus(),
            FullyConnectedLayer(self.hidden_dim, 19, lr_multiplier=options['decoder_lr_mul'])
            )      
        elif network_type == 'chin':
            self.geo_net_appdx = torch.nn.Sequential(
            FullyConnectedLayer(1 + options['decoder_seg_dim']+32+3, self.hidden_dim, lr_multiplier=options['decoder_lr_mul']),
            torch.nn.Softplus(),
            FullyConnectedLayer(self.hidden_dim, 20, lr_multiplier=options['decoder_lr_mul'])
            )
        else:
            print("wrong")
    ...
    def forward(self, sampled_norm_features, sampled_denorm_features, ray_directions):
        sampled_norm_features = sampled_norm_features.mean(1) # torch.Size([4, 1572864, 32])
        sampled_denorm_features = sampled_denorm_features.mean(1)
        ...
        N, M, C = x.shape
        x = x.view(N*M, C) # torch.Size([6291456, 32])
        
        Nd, Md, Cd = ray_directions.shape
        direct = ray_directions.view(Nd*Md, Cd)
        
        y = self.geo_net(x)
        if self.network_type=='simple':
            y = self.softplus(y)
            x = self.geo_net_appdx(y)
        elif self.network_type=='base' or self.network_type=='nose' or self.network_type=='eyes' or self.network_type=='attn'or self.network_type=='chin':
            concat_x = torch.cat((x,y),dim=1)
            concat_x = self.softplus(concat_x)
            concat_with_dir = torch.cat((concat_x,direct),dim=1)
            x = self.geo_net_appdx(concat_with_dir) 
        else:
            print('network type is wrong!')
        x = x.view(N, M, -1)
        y = y.view(N, M, -1)
        sigma = y[..., 0:1]
