...
class DisentangledOSGDecoder(torch.nn.Module):
    def __init__(self, n_features, options,network_type='simple'):
        ...
        if network_type=='simple':
            self.ngeo_net = torch.nn.Sequential(
            FullyConnectedLayer(n_features, self.hidden_dim, lr_multiplier=options['decoder_lr_mul']),
            torch.nn.Softplus(),
            FullyConnectedLayer(self.hidden_dim, options['decoder_seg_dim']+2, lr_multiplier=options['decoder_lr_mul'])
            )
        elif network_type=='eyes' or network_type=='nose': 
            self.ngeo_net = torch.nn.Sequential(
            FullyConnectedLayer(n_features, self.hidden_dim, lr_multiplier=options['decoder_lr_mul']),
            torch.nn.Softplus(),
            FullyConnectedLayer(self.hidden_dim, options['decoder_seg_dim']+4, lr_multiplier=options['decoder_lr_mul'])
            )
    ...
    def forward(self, sampled_norm_features, sampled_denorm_features, ray_directions):
        ...
        x_prime = self.geo_net(x)
        x_prime = x_prime.view(N, M, -1)
        sigma = x_prime[..., 0:1]
